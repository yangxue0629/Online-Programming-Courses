...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText)
	{
		...
		starter = words[0].toLowerCase();
		String preword = starter;
		for(int i = 1; i <= words.length -1; i++){
			
			int index = -1;
			for(int j = 0; j < wordList.size(); j++)
				if(wordList.get(j).getWord().equals(preword)){
					index = j;
					break;
				}
			if(index >= 0){
				wordList.get(index).addNextWord(words[i].toLowerCase());
			}
			else{
				ListNode n = new ListNode(preword);
				n.addNextWord(words[i].toLowerCase());
				wordList.add(n);
			}
			preword = words[i];
		}	
	...
	public String generateText(int numWords) {
		...
		while(count <= numWords){
			for(int i = 0; i < wordList.size();i++){
				if(wordList.get(i).getWord().toLowerCase().equals(currWord)){
					currWord = wordList.get(i).getRandomNextWord(this.rnGenerator);
					output += currWord + " ";
					count++;
				}
			}
